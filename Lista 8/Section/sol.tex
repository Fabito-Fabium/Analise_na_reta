...
%<*ex6a>
\textbf{Dem:} Ora, afim de contradição suponhamos que $x \in \overline A$, portanto, existe uma sequência $\sqn{x}$, com $x_n \in A$ para todo $n \inN$, tal que $\limsq n \infty x = x$. Da definição de limite, para todo $\epsilon > 0$ existe $n_0 \inN$, tal que $|x_n - x| < \epsilon$ para todo inteiro $n \ge n_0$, ou seja, em particular, para todo $\epsilon > 0$, existe $n_0 \inN$ tal que $x - \epsilon < x_{n_0} < x + \epsilon$, deste modo, para todo $\epsilon$ existe $\tau \in A$, tal que $\tau \in (x - \epsilon, x + \epsilon)$, $(\ast)$.
\\
...
\end{ceqnalign*}
e, mais ainda, que todo número real menor que $1$ não está em $A$, segue que existe $\tau \in A$ tal que $\tau \in \xssy{x}{\epsilon} \not \subset A$, isto é, $\tau \in A$ e $\tau \not \in A$, uma contradição, portanto, para todo $x<1$, temos $x \not \in \overline A$, como queríamos.
\\
Para o caso $x>3$. Fixando $\epsilon = \frac{x - 3}{2} > 0$, e, de $(\ast)$, tendo em vista que
\begin{ceqnalign*}
	3 = \frac{3+3}{2}<\frac{3+x}{2} = x - \frac{x - 3}{2} = x - \epsilon
\end{ceqnalign*}
e, que todo número real maior que $3$ não está em $A$, segue que existe $\tau \in A$ tal que $\tau \in \xssy{x}{\epsilon} \not \subset A$, isto é,$\tau \in A$ e $\tau \not \in A$, uma contradição, portanto, para todo $x>3$, temos $x \not \in \overline{A}$, como queríamos.
\\
...
%<*ex6b>
Ora, afim de contradição, suponhamos que $x \in A'$, isto é, existe uma sequência $\sqn x$ com $x_n \in A/\{x\}$ tal que $\limsq n \infty x = x$. Da definição de limite, temos que para todo $\epsilon>0$ existe $n_0 \inN$ tal que $\asbeps {x_n} x$ com $x \inN_{\ge n_0}$. Ou seja, em particular, tomando $\tau = x_{n_0}$, para todo $\epsilon>0$, existe $\tau \in A/\{x\}$ tal que $\asbeps \tau x$, ($\ast$).
\\
Se $x > 3$, fixando $\epsilon = \frac{x-3}{2} > 0$, de $(\ast)$, existe $\tau \in A/\{x\}$ tal que $\tau \in \xssy{x} {\frac{x-3}{2}}$, portanto $\tau \in \parth{\frac{x+3}{2}, \frac{3x-3}{2}}$. Mas, como
\begin{ceqnalign*}
	3 = \frac{3+3}{2} < \frac{x+3}{2}
\end{ceqnalign*}
segue que $\parth{\frac{x+3}{2}, \frac{3x-3}{2}} \cap A/\{x\} = \emptyset$, ou seja $\tau \in \emptyset$, contradição, como queríamos.
\\
Se $x = 3$, fixando $\epsilon = \frac{1}{2} > 0$, de $(\ast)$, temos que existe $\tau \in A/\{x\}$ tal que $\tau \in \xssy {x}{\frac{1}{2}}$, mas, por construção, $\xssy x {\frac{1}{2}} \cap A/\{x\} = \emptyset$, portanto $\tau \in \emptyset$, uma contradição, como queríamos.
\\
Se $2<x<3$
