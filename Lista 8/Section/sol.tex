...
%<*ex6a>
\textbf{Dem:} Primeiro, vejamos que para todo $x<1$, $x \not \in \overline A$. Ora, afim de contradição suponhamos que $x \in \overline A$, portanto, existe uma sequência $\sqn{x}$, com $x_n \in A$ para todo $n \inN$, tal que $\limsq n \infty x = x$. Da definição de limite, para todo $\epsilon > 0$ existe $n_0 \inN$, tal que $|x_n - x| < \epsilon$ para todo inteiro $n \ge n_0$, ou seja, em particular, para todo $\epsilon > 0$, existe $n_0 \inN$ tal que $x - \epsilon < x_{n_0} < x + \epsilon$, deste modo, para todo $\epsilon$ existe $\tau \in A$, tal que $\tau \in (x - \epsilon, x + \epsilon)$. Note que, sabendo que $x<1$, podemos fixar $\epsilon = \frac{1-x}{2} > 0$ e, tendo em vista que
\begin{ceqnalign*}
